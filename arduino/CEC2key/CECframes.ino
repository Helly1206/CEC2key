...
CECframes::CECframes(int address) {
  MonitorMode = false;
  Promiscuous = false;
...
void CECframes::Initialize() {
  _lastLineState = LineState();
  _lastStateChangeTime = micros();
}

void CECframes::SetAddress(int address) {
  _address = address & 0x0f;
}

bool CECframes::Raise() {
  if (MonitorMode)
    return LineState();
...
void CECframes::HasRaised(unsigned long time) {
  _lastLineState = true;
  _lastStateChangeTime = time;
}

void CECframes::HasLowered(unsigned long time) {
  _lastLineState = false;
  _lastStateChangeTime = time;
  _bitStartTime = time;
}

bool CECframes::CheckAddress() {
  if (ReceivedBytes() == 1) {
...
void CECframes::ReceivedBit(bool state) {
...
unsigned long CECframes::LineError() {
...
unsigned long CECframes::Process() {
  ...
  /*Serial.print("Sh:");
  Serial.print(_primaryState);
  Serial.print(",");
  Serial.print(_secondaryState);
  Serial.print(",");
  Serial.println(_tertiaryState);*/
  ...
  if( currentLineState == lastLineState && !(_primaryState == CEC_TRANSMIT || (_primaryState == CEC_RECEIVE && (_secondaryState == CEC_RCV_ACK_SENT || _secondaryState == CEC_RCV_LINEERROR))))
    return waitTime;
...
//DbgPrint("2: %ld %ld\n", difftime, micros());
  ...
  case CEC_TRANSMIT:
    if (lastLineState != currentLineState) {
      ...
      // ACK state
      if (_secondaryState != CEC_XMIT_ACK && _secondaryState != CEC_XMIT_ACK2 && _secondaryState != CEC_XMIT_ACK3 && _secondaryState != CEC_XMIT_ACK_TEST) {
...
void CECframes::ResetTransmit(bool retransmit) {
...
void CECframes::OnTransmitBegin() {
  if (!MonitorMode) {
...
void CECframes::OnTransmitComplete(bool success) {
}

void CECframes::OnReceiveComplete(unsigned char* buffer, int count) {
  ResetReceiveBuffer();
}

void CECframes::ClearTransmitBuffer() {
  _transmitBufferCount = 0;
  _transmitBufferBit = 7;
  _transmitBufferByte = 0;
}

bool CECframes::Transmit(unsigned char* buffer, int count) {
...
bool CECframes::TransmitPartial(unsigned char* buffer, int count) {
...
int CECframes::RemainingTransmitBytes() {
  return _transmitBufferCount - _transmitBufferByte;
}

int CECframes::TransmitSize() {
  return _transmitBufferCount;
}

void CECframes::PushReceiveBit(int bit) {
...
int CECframes::ReceivedBytes() {
  return _receiveBufferByte;
}

void CECframes::ResetReceiveBuffer() {
  _receiveBufferBit = 7;
  _receiveBufferByte = 0;
}

void CECframes::ResetTransmitBuffer() {
  _transmitBufferBit = 7;
  _transmitBufferByte = 0;
}
